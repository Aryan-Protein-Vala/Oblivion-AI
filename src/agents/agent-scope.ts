...
type AgentEntry = NonNullable<NonNullable<OblivionConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: OblivionConfig): AgentEntry[] {
...
export function listAgentIds(cfg: OblivionConfig): string[] {
...
export function resolveDefaultAgentId(cfg: OblivionConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: OblivionConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: OblivionConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: OblivionConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
...
export function resolveAgentDir(cfg: OblivionConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
