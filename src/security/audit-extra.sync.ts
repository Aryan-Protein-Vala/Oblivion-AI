...
function summarizeGroupPolicy(cfg: OblivionConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: OblivionConfig): boolean {
...
function collectModels(cfg: OblivionConfig): ModelRef[] {
  const out: ModelRef[] = [];
  addModel(
    out,
...
function listKnownNodeCommands(cfg: OblivionConfig): Set<string> {
  const baseCfg: OblivionConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: OblivionConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: OblivionConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: OblivionConfig): boolean {
...
function isBrowserEnabled(cfg: OblivionConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: OblivionConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
function listPotentialMultiUserSignals(cfg: OblivionConfig): string[] {
  const out = new Set<string>();
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return [];
...
function collectRiskyToolExposureContexts(cfg: OblivionConfig): {
...
export function collectAttackSurfaceSummaryFindings(cfg: OblivionConfig): SecurityAuditFinding[] {
...
export function collectSecretsInConfigFindings(cfg: OblivionConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
...
export function collectSandboxDockerNoopFindings(cfg: OblivionConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: OblivionConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
...
export function collectNodeDenyCommandPatternFindings(cfg: OblivionConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: OblivionConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: OblivionConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: OblivionConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectLikelyMultiUserSetupFindings(cfg: OblivionConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
