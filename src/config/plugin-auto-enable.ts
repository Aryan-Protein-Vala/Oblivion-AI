...
import type { OblivionConfig } from "./config.js";
...
function isWhatsAppConfigured(cfg: OblivionConfig): boolean {
...
function isGenericChannelConfigured(cfg: OblivionConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: OblivionConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: OblivionConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: OblivionConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: OblivionConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: OblivionConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: OblivionConfig, pluginId: string): OblivionConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
