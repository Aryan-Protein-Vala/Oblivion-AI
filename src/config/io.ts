...
import type { OblivionConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
function unsetPathForWrite(
  root: OblivionConfig,
  pathSegments: string[],
): { changed: boolean; next: OblivionConfig } {
...
function coerceConfig(value: unknown): OblivionConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as OblivionConfig;
}
...
function stampConfigVersion(cfg: OblivionConfig): OblivionConfig {
...
function warnIfConfigFromFuture(cfg: OblivionConfig, logger: Pick<typeof console, "warn">): void {
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as OblivionConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): OblivionConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as OblivionConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: OblivionConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as OblivionConfig)
        : cfgToWrite;
...
let runtimeConfigSnapshot: OblivionConfig | null = null;
let runtimeConfigSourceSnapshot: OblivionConfig | null = null;
...
export function getRuntimeConfigSnapshot(): OblivionConfig | null {
  return runtimeConfigSnapshot;
}

export function loadConfig(): OblivionConfig {
