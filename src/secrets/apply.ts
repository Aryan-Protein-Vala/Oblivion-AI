...
import { resolveStateDir, type OblivionConfig } from "../config/config.js";
...
function setByPathSegments(root: OblivionConfig, segments: string[], value: unknown): boolean {
  if (segments.length === 0) {
...
function deleteByPathSegments(root: OblivionConfig, segments: string[]): boolean {
  if (segments.length === 0) {
    return false;
  }
  let cursor: Record<string, unknown> = root as unknown as Record<string, unknown>;
...
function collectAuthStorePaths(config: OblivionConfig, stateDir: string): string[] {
