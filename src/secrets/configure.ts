...
function getSecretProviders(config: OblivionConfig): Record<string, SecretProviderConfig> {
...
function removeSecretProvider(config: OblivionConfig, providerAlias: string): boolean {
  if (!isRecord(config.secrets?.providers)) {
    return false;
  }
...
function buildCandidates(config: OblivionConfig): ConfigureCandidate[] {
...
function toSourceChoices(config: OblivionConfig): Array<{ value: SecretRefSource; label: string }> {
...
async function configureProvidersInteractive(config: OblivionConfig): Promise<void> {
  while (true) {
...
function collectProviderPlanChanges(params: { original: OblivionConfig; next: OblivionConfig }): {
