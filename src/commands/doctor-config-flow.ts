...
function stripUnknownConfigKeys(config: OblivionConfig): {
  config: OblivionConfig;
  removed: string[];
} {
...
function noteOpencodeProviderOverrides(cfg: OblivionConfig) {
...
export function collectMissingDefaultAccountBindingWarnings(cfg: OblivionConfig): string[] {
...
function scanTelegramAllowFromUsernameEntries(cfg: OblivionConfig): TelegramAllowFromUsernameHit[] {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: OblivionConfig): Promise<{
  config: OblivionConfig;
  changes: string[];
}> {
...
function scanDiscordNumericIdEntries(cfg: OblivionConfig): DiscordNumericIdHit[] {
...
function maybeRepairDiscordNumericIds(cfg: OblivionConfig): {
  config: OblivionConfig;
  changes: string[];
} {
...
function scanMutableAllowlistEntries(cfg: OblivionConfig): MutableAllowlistHit[] {
 ...
 */
function maybeRepairOpenPolicyAllowFrom(cfg: OblivionConfig): {
  config: OblivionConfig;
  changes: string[];
} {
...
async function maybeRepairAllowlistPolicyAllowFrom(cfg: OblivionConfig): Promise<{
  config: OblivionConfig;
  changes: string[];
}> {
  const channels = cfg.channels;
 ...
 * file support.
 */
function detectEmptyAllowlistPolicy(cfg: OblivionConfig): string[] {
  const channels = cfg.channels;
  if (!channels || typeof channels !== "object") {
    return [];
  }
...
function collectExecSafeBinScopes(cfg: OblivionConfig): ExecSafeBinScopeRef[] {
...
function scanExecSafeBinCoverage(cfg: OblivionConfig): ExecSafeBinCoverageHit[] {
...
function scanExecSafeBinTrustedDirHints(cfg: OblivionConfig): ExecSafeBinTrustedDirHintHit[] {
...
function maybeRepairExecSafeBinProfiles(cfg: OblivionConfig): {
  config: OblivionConfig;
  changes: string[];
  warnings: string[];
} {
...
function scanLegacyToolsBySenderKeys(cfg: OblivionConfig): LegacyToolsBySenderKeyHit[] {
  const hits: LegacyToolsBySenderKeyHit[] = [];
  collectLegacyToolsBySenderKeyHits(cfg, [], hits);
  return hits;
}

function maybeRepairLegacyToolsBySenderKeys(cfg: OblivionConfig): {
  config: OblivionConfig;
  changes: string[];
} {
  const next = structuredClone(cfg);
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: OblivionConfig = baseCfg;
