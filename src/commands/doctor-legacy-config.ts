...
export function normalizeCompatibilityConfigValues(cfg: OblivionConfig): {
  config: OblivionConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: OblivionConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as OblivionConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as OblivionConfig["browser"],
    };
    changes.push(
