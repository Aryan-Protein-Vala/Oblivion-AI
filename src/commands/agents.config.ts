...
function resolveAgentName(cfg: OblivionConfig, agentId: string) {
...
function resolveAgentModel(cfg: OblivionConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: OblivionConfig): AgentSummary[] {
