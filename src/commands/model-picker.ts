...
type PromptDefaultModelResult = { model?: string; config?: OblivionConfig };
...
function resolveConfiguredModelRaw(cfg: OblivionConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: OblivionConfig): string[] {
...
export function applyPrimaryModel(cfg: OblivionConfig, model: string): OblivionConfig {
...
export function applyModelAllowlist(cfg: OblivionConfig, models: string[]): OblivionConfig {
  const defaults = cfg.agents?.defaults;
