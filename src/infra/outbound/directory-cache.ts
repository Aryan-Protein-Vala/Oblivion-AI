...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: OblivionConfig | null = null;
  ...
  get(key: string, cfg: OblivionConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: OblivionConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: OblivionConfig): void {
