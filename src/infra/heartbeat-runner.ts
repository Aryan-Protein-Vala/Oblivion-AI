...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: OblivionConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: OblivionConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: OblivionConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: OblivionConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: OblivionConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: OblivionConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: OblivionConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
