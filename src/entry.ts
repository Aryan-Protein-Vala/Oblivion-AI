...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "oblivion.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "oblivion.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    child.once("error", (error) => {
      console.error(
        "[oblivion] Failed to respawn CLI:",
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[oblivion] ${parsed.error}`);
      process.exit(2);
    }
